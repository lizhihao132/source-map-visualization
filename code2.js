(() => {
  ...
  */
    ...
    async function createTextArea({
        ...
        function boxForRange(dx, dy, columnWidth, {
            startColumn,
            endColumn
        }) {
        ...
        return {
            ...
            getHoverRect() {
                ...
                const {
                    firstMapping,
                    rangeOfMapping
                } = analyzeLine(lineIndex, column, column, 'floor');
            ...
            scrollTo(index, line) {
                ...
                const {
                    indexToColumn
                } = analyzeLine(line, index, index, 'floor');
                const column = indexToColumn(index);
                const {
                    firstMapping,
                    rangeOfMapping
                } = analyzeLine(line, column, column, 'floor');
                const range = rangeOfMapping(firstMapping);
                ...
                const drawRow = (dx, dy, lineIndex, firstColumn, lastColumn) => {
                    ...
                    if (firstRun < runCount) {
                        ...
                        for (let i = firstRun; i <= lastRun; i++) {
                            ...
                            if (text === void 0) {
                                text = runText[i] = !whitespace ? raw.slice(run_startIndex(run), run_endIndex(run)) :
                ...
                if (_switch_view) {
                    ...
                    if (gw > (generatedBounds.width - gx)) {
                        ...
                        while (x2 > generatedBounds.width) {
                            i++;
                            x2 -= generatedBounds.width - 89;	// 89 是 canvas 大框左边线到 canvas 左边框左边线 的距离.
                        }
