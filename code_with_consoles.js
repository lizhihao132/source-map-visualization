(() => {
  ...
  */
  ...
  async function finishLoading(code, map) {
    ...
    isInvalid = true;
	console.info('finish createelement of option')
	
    updateHash(code, map);
	
	console.info('update hash finish')

    // Let the browser update before parsing the source map, which may be slow
	console.info('begin to wait for dom finished')
    await waitForDOM();
	...
	console.info('total chars length:', totalChars)
	...
	console.info('otherSource:', otherSource, 'originalName:', originalName, 'sm.sources.length:', sm.sources.length)
    ...
    if (sm.sources.length > 0) {
      const updateOriginalSource = (sourceIndex, progress) => {
        ...
        return createTextArea({
		  ...
		  bounds() {
			console.info('original bounds called', _switch_view, sourceIndex, 'source.content.length:', source.content.length)
		    if(_switch_view){
      ...
      fileList.onchange = async () => {
		...
		console.info('await updateOriginalSource finished')
        isInvalid = true;
      };
      finalOriginalTextArea = await updateOriginalSource(0, progress);
	  console.info('await updateOriginalSource 0, progress finished')
    }
    ...
    generatedTextArea = await createTextArea({
	  ...
	  bounds() {
		console.info('generated bounds called', _switch_view)
		if(_switch_view){
			return {
			  x: 0,
			  y: toolbarHeight,
	...
	console.info('ok, we get here', sm.sources.length)
